...
| `--expert-pool-miss-method METHOD` | how the pool runs a miss:<br/>- cpu-serial: the CPU miss chain stays one split (default)<br/>- cpu-parallel: the mount chain is handed to the device ahead of the CPU chain<br/>- gpu: compute the miss rows on the device, below the offload batch threshold as well (requires a non-host backend; not a speed-up there) |
